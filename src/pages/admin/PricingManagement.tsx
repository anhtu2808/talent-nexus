...
import { 
  ...
  Image as ImageIcon, Search, User, Building2, Zap, LayoutGrid
} from "lucide-react";
import { cn } from "@/lib/utils";

const initialPlans = [
  { id: 1, name: "Student AI Booster", originalPrice: 897000, salePrice: 199000, target: "candidate", type: "AI Credit", duration: 90, status: "Active" },
  { id: 2, name: "Enterprise Hiring Pro", originalPrice: 3588000, salePrice: 1359000, target: "recruiter", type: "Job Package", duration: 270, status: "Active" },
  { id: 3, name: "Quick Scan Pack", originalPrice: 50000, salePrice: 25000, target: "candidate", type: "AI Scan", duration: 30, status: "Active" },
];
...
export default function PricingManagement() {
  ...
  // Logic lọc: Nếu tab là 'all' thì hiện tất cả, ngược lại lọc theo target
  ...
  return (
    <div className="space-y-6 text-left">
      {/* 1. Integrated Navigation & Action Bar */}
      <Tabs defaultValue="all" className="w-full" onValueChange={setActiveTab}>
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-8">
          {/* Left: Enhanced 3-Tab System + All Plans Tab */}
          <TabsList className="bg-slate-100/50 p-1 rounded-xl border border-slate-200">
            <TabsTrigger value="all" className="rounded-lg font-bold px-5 data-[state=active]:bg-white data-[state=active]:text-[#0F2238] gap-2 shadow-none transition-all">
              <LayoutGrid size={15} /> All Plans
            </TabsTrigger>
            <TabsTrigger value="candidate" className="rounded-lg font-bold px-5 data-[state=active]:bg-white data-[state=active]:text-blue-600 gap-2 shadow-none transition-all">
              <User size={15} /> Candidates
            </TabsTrigger>
            <TabsTrigger value="recruiter" className="rounded-lg font-bold px-5 data-[state=active]:bg-white data-[state=active]:text-purple-600 gap-2 shadow-none transition-all">
              <Building2 size={15} /> Recruiters
            </TabsTrigger>
          </TabsList>
          
          {/* Right: Search + Create Button Group */}
          <div className="flex items-center gap-3 w-full lg:w-auto">
            <div className="relative group flex-1 md:w-64">
              ...
              <Input 
                ...
                className="pl-10 h-11 rounded-xl bg-white border-slate-200 focus-visible:ring-[#38B65F]" 
              />
            </div>

            <Dialog open={open} onOpenChange={setOpen}>
              <DialogTrigger asChild>
                <Button className="bg-[#38B65F] hover:bg-[#2D914C] text-white font-bold rounded-xl px-5 gap-2 h-11 shadow-lg shadow-green-500/20 whitespace-nowrap">
              ...
              <DialogContent className="sm:max-w-[650px] rounded-3xl p-0 overflow-hidden border-none shadow-2xl">
                <DialogHeader className="p-8 pb-0 bg-slate-50/50">
                  <DialogTitle className="text-2xl font-black text-[#0F2238]">New Subscription Tier</DialogTitle>
                </DialogHeader>
                ...
                <div className="p-8 space-y-6">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Input placeholder="e.g. Premium Monthly" className="rounded-xl bg-slate-50 border-slate-100 h-11" />
                    </div>
                    <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    <Label className="text-sm font-bold text-slate-700">Package Benefits (Rich Text)</Label>
                    <div className="rounded-xl border border-slate-200 overflow-hidden bg-white">
                      ...
                      <Textarea placeholder="List the benefits..." className="border-none focus-visible:ring-0 min-h-[120px] text-sm p-4" />
                    </div>
                  </div>
                  <div className="grid grid-cols-3 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input type="number" placeholder="0" className="rounded-xl h-11 font-bold text-[#38B65F]" />
                    </div>
                    <div className="space-y-2">
                ...
                <DialogFooter className="p-8 pt-0 gap-3">
                  ...
                  <Button className="bg-[#38B65F] hover:bg-[#2D914C] text-white font-black rounded-xl px-10 h-11">Publish Plan</Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
          </div>
        ...
        {/* 2. Content Area */}
        <TabsContent value={activeTab} className="grid grid-cols-1 gap-4 focus-visible:outline-none">
            ...
            <Card key={plan.id} className="border border-slate-100 shadow-sm hover:shadow-md transition-all rounded-2xl overflow-hidden group bg-white">
              <CardContent className="p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    <div className="flex items-center gap-3">
                      ...
                      {/* Hiển thị label phân loại khi ở tab All Plans */}
                      {activeTab === "all" && (
                        <Badge variant="outline" className="text-[10px] border-slate-200 text-slate-400 font-bold uppercase">
                          {plan.target}
                        </Badge>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-4 mt-2">
                      <div className="flex flex-col">
                        <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest leading-none mb-1">Selling Price</span>
                ...
                <div className="flex items-center gap-3 w-full md:w-auto border-t md:border-t-0 pt-4 md:pt-0">
                  <Button variant="outline" className="flex-1 md:flex-none rounded-xl border-slate-200 font-bold hover:bg-slate-50 gap-2 h-10 px-5">
                    <Edit2 size={16} className="text-slate-400" /> Edit
                  </Button>
                  <Button variant="outline" className="h-10 w-10 p-0 rounded-xl border-slate-200 hover:bg-red-50 hover:text-red-500 transition-colors">
                    <Trash2 size={16} />
                  </Button>
                </div>
              </CardContent>
          ...
          {filteredPlans.length === 0 && (
            <div className="py-20 text-center border-2 border-dashed border-slate-100 rounded-3xl bg-slate-50/30">
              <p className="text-slate-400 font-medium italic">No plans found.</p>
            </div>
          )}
        </TabsContent>
      </Tabs>
